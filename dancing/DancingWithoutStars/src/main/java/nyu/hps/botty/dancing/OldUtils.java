    ...
    static List<Point>[] generatePaths1(Point[][] startEndP, String[][] grid) {
        ...
        while (true) {
            ...
            for (int ind: byL) {
                ...
                PriorityQueue<Utils.Edge> dq = new PriorityQueue<>(Comparator.comparingInt(Utils.Edge::getDist));

                Point start = currentLocs[ind];
                dq.add(new Utils.Edge(start.x, start.y, 0));
                ...
                while (!dq.isEmpty()) {
                    ...
                    for (int[] dir : moves) {
                        ...
                        if (dist[newX][newY] > newDist) {
                            Utils.Edge newEdge = new Utils.Edge(newX, newY, newDist);
