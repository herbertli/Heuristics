 ...
 */
public class RedemptionChoreographer extends Choreographer {
    ...
    public void solve() {
        ...
        allPairsShortestPaths(boardSize, starGrid, dist);
        int iterations = 0;
        ...
        while (Instant.now().isBefore(globalEnd)) {
            ...
            if (paths == null || minTurns > newPaths[0].size()) {
                System.out.println("Found a good solution!");
    ...
    static void allPairsShortestPaths(int boardSize, boolean[][] starGrid, int[][] dist) {
        Instant startTime = Instant.now();
        int bs = boardSize;
        int n = bs * bs;
        for(int i = 0; i < n; i++) {
        ...
        for(int i = 0; i < bs; i++) {
            for(int j = 0; j < bs; j++) {
                q.add(i); q.add(j);
                if(starGrid[i][j]) continue;
                dist[i*bs+j][i*bs+j] = 0;
                while(!q.isEmpty()) {
                    x = q.poll(); y = q.poll();
                    for(int[] move : Utils.moves) {
                        if(x + move[0] >= 0 && 
                           x + move[0] < bs &&
                           y + move[1] >= 0 &&
                           y + move[1] < bs) {
                            nx = x + move[0];
                            ny = y + move[1];
                            if(starGrid[nx][ny]) continue;
                            if(dist[i*bs+j][x*bs+y] + 1 < dist[i*bs+j][nx*bs+ny]) {
                                dist[i*bs+j][nx*bs+ny] = dist[i*bs+j][x*bs+y] + 1;
                                q.add(nx); q.add(ny);
                            }
                        }
                    }
        ...
        System.out.println("Time to run APSP = " + (Instant.now().toEpochMilli() - startTime.toEpochMilli()) / 1000.0);
    }
}
