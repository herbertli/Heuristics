...
class GameServer(object):
    def __init__(self, d=36, y=6, r=6, m=10, L=4, p=2):
        ...
        self.player_attributes = [json.loads(
            info) for info in self.server.receive_from_all()]
    ...
    def complete_submarine_move(self, submarine_move):
        ...
        self.submarine_location = (
            self.submarine_location + submarine_move) % 100
        self.is_submarine_in_red = self.submarine_location in self.red_alert
    ...
    def deploy_probes(self, probes):
        ...
        for probe in probes:
            ...
            range_start = (probe - self.L) if (probe -
                                               self.L) > 0 else (100 + (probe - self.L))
            probe_range = [i % 100 for i in range(
                range_start, range_start + (self.L * 2) + 1)]
    ...
    def play_game(self):
        ...
        trench_probe_move, trench_time_spent = self.timed_request(
            {'d': self.d, 'y': self.y, 'r': self.r,
                'm': self.m, 'L': self.L, 'p': self.p},
            self.trench_idx
        )
